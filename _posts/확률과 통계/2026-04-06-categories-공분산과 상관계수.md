...
즉, 한 변수가 증가할 때 다른 변수도 함께 증가하는지, 혹은 감소하는지를 확인할 수 있다.

---

### 수식

모집단 공분산은 다음과 같이 정의된다.(편차들의 곱의 평균)
...
- **공분산($Cov(X, Y)$) $> 0$**
    ...
    - 예: 키와 몸무게(기본적으로 키가 클수록 몸무게가 무겁다)
- **공분산($Cov(X, Y)$)  $< 0$**
    - $X$가 증가할 때 $Y$는 감소하는 경향이 있다.
    - 예: 스마트폰 사용 시간과 수면 시간(스마트폰을 많이 쓰는 사람은 보통 수면시간이 적다)
- **공분산($Cov(X, Y)$)  $≈ 0$**
...
예를 들어 키를 **cm**로 측정할지 **m**로 측정할지에 따라 값이 크게 달라질 수 있다.

즉, 값의 절대 크기만으로 관계의 강도를 직접 비교하기 어렵다. 그러므로 공분산은 상관계수를 이용하여 해석을 한다.
...
공분산의 단위 의존성 문제를 해결하기 위해 각 변수의 표준편차로 나누어 정규화한다.
